...
class _AIRecommendationsPageState extends State<AIRecommendationsPage> {
  ...
  Future<void> _sendTravelPlan(BuildContext context) async {
  // 1) show “busy” bar
  _showProgressSnackBar('Preparing travel plan for sharing…');

  try {
    final parsed = parseRecommendations();
    await PdfGenerator.generateAndSharePdf(parsed);
    // 2) hide it as soon as the PDF is ready
    scaffoldMessengerKey.currentState?.hideCurrentSnackBar();
  } catch (e) {
    // hide the “busy” bar so we can show the error
    scaffoldMessengerKey.currentState?.hideCurrentSnackBar();
